...
@section('content')
    ...
    <form action="{{ route('participants.update', $participant->id) }}" method="POST">
        ...
        <div class="mb-3">
            <label>Name</label>
            {{-- LOGIC: Try to use 'old' input first. If empty, use database value ($participant->name) --}}
            <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror"
                   value="{{ old('name', $participant->name) }}" minlength="3" placeholder="Name (minimum 3 char, no special characters)" required>
        ...
        <div class="mb-3">
            ...
            <input type="email" name="email" id="email" class="form-control @error('email') is-invalid @enderror"
                   value="{{ old('email', $participant->email) }}" placeholder="Email (valid email required)" required>
        ...
        <div class="mb-3">
            ...
            <input type="tel" name="phone" id="phone" class="form-control @error('phone') is-invalid @enderror"
                   value="{{ old('phone', $participant->phone) }}" pattern="[0-9]+" placeholder="085212341234" title="Numbers only" required>
        ...
        <hr>
        <h4>Manage Class Enrollments</h4>
        <div class="row">
                    ...
                    <div class="card p-2">
                        <div class="form-check">
                            ...
                            <input class="form-check-input" type="checkbox"
                                   ...
                                   @else
                                       {{ $participant->courses->contains($course->id) ? 'checked' : '' }}
                                   @endif
                                   >
        ...
        <button type="submit" class="btn btn-primary mt-3">Update Participant</button>
        <a href="{{ route('participants.index') }}" class="btn btn-secondary mt-3">Cancel</a>
    </form>

    <script>
        // Real-time validation for Participants Edit Form
